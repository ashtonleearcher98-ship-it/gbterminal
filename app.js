...
function pending() {
  ...
  return `<div class="pending"><div class="pulse">⌁</div><span class="pill online">● &nbsp;Awaiting customer approval</span><div class="pending-total">${amount(r.amountMinor)}</div><p>Open <b>Greenwick Bank → Payments → Nearby pay</b> on the customer's phone. Enter this code and review the merchant and amount.</p><div class="token">${esc(r.token)}</div><button class="outline" data-copy>Copy code</button><p class="fine">Request expires ${date(r.expiresAt)}. This browser cannot broadcast the Android BLE signal; the code resolves the same request.</p><button class="danger wide" data-cancel ${state.busy ? 'disabled' : ''}>Cancel request</button></div>`;
}
